...
var notice = '<div style="padding: 0.3em 0.3em; padding-top: 0;"><div style="background: #3e3e3e; padding: 0.5em; border-radius: 1em;"><div style="line-height: 1.2;"><span style="color: #f30000"><div style="text-align: center;">Плагины TMDB Proxy<br> СТАВИМ ТОЛЬКО ПО ОДНОМУ</div></span></div></div></div>';
...
/* TMDB Proxy */
		Lampa.SettingsApi.addParam({
					component: 'add_plugin',
					param: {
						name: 'add_tmdb_proxy_plugin',
      		                        	type: 'static',
						default: true
                        		},
					field: {
                                name: icon_add_tmdb_proxy_plugin
                        },
					onRender: function(item) {
						item.on('hover:enter', function () {
							Lampa.Settings.create('add_tmdb_proxy_plugin');
                                                ...
                                                var pluginsArray = Lampa.Storage.get('plugins');
                                                    setTimeout(function() {
                                                       ...
                                                       if (myResult && pluginStatus !== 0) {
                                                          $('div[data-name="ByLAMPA TMDB"]').find('.settings-param__status').removeClass('active error').addClass('active');
                                                       } else if (pluginStatus === 0) {
                                                          $('div[data-name="ByLAMPA TMDB"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                                                       } else {
                                                          $('div[data-name="ByLAMPA TMDB"]').find('.settings-param__status').removeClass('active error').addClass('error');
                                                       }
                                                    }, 100);
						...
						var myResult = checkPlugin('http://plugin.rootu.top/tmdb.js');
                                                var pluginsArray = Lampa.Storage.get('plugins');
                                                    setTimeout(function() {
                                                       ...
                                                       if (myResult && pluginStatus !== 0) {
                                                          $('div[data-name="Rootu TMDB"]').find('.settings-param__status').removeClass('active error').addClass('active');
                                                       } else if (pluginStatus === 0) {
                                                          $('div[data-name="Rootu TMDB"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                                                       } else {
                                                          $('div[data-name="Rootu TMDB"]').find('.settings-param__status').removeClass('active error').addClass('error');
                                                       }
                                                    }, 100);
						    item.on("hover:enter", function (event) {
                                                        nthChildIndex = focus_back(event); // Сохраняем элемент в переменной
...
/* Интерфейс */
        Lampa.Settings.listener.follow('open', function (e) {
            if (e.name == 'main') {
				...
				setTimeout(function() {
					$('div[data-component="add_tmdb_proxy_plugin"]').remove();
	    ...
	    Lampa.SettingsApi.addParam({
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагин отображает текущее состояние сериала (сезон/серия). Отключить/включить можно в настройках приложения'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Аниме',
						description: 'Плагин добавляет в левое меню подборки аниме от TMDB'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Цветные оценки',
						description: 'Плагин окрашивает оценки на карточках в зависимости от значения'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Трейлеры Itunes',
						description: 'Плагин позволяет из карточки смотреть трейлеры с Itunes'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Трейлеры Rutube',
						description: 'Плагин позволяет из карточки смотреть трейлеры с Rutube'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
                ...
                Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагин меняет размеры интерфейса'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Статистика',
						description: 'Плагин для ведения статистики просмотра фильмов в приложении, а в конце года - генерация странички с итогами года'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Лейбл сериала',
						description: 'Плагин заменяет названия лейблов на карточках сериалов (с TV на Сериал)'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Card elems center',
						description: 'Плагин центрирует элементы в карточке для мобильной версии приложения'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Трейлеры',
						description: 'Плагин добавляет на главную страницу пункт меню Трейлеры с различными разделами (Популярные фильмы, В прокате, Скоро в кино, Популярные сериалы, Новые сериалы и сезоны, Скоро на ТВ)'
					},
					onChange: function(value) {
						if (value == '1') {
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Trailer off',
						description: 'Плагин убирает трейлеры в карточке'
					},
					onChange: function(value) {
						if (value == '1') {
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Quality',
						description: 'Плагин ставит отметки качества фильмов на карточки и внутри них'
					},
					onChange: function(value) {
						if (value == '1') {
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагин меняет источник рейтинга на главной странице через настройки интерфейса'
					},
					onChange: function(value) {
						if (value == '1') {
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагин показывает внутри карточки оригинальное название без перевода'
					},
					onChange: function(value) {
						if (value == '1') {
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Top ByLAMPA',
						description: 'Плагин создает в левом меню пункт Top, для просмотра самого интересного контента на основе оценок пользователей приложения ByLAMPA'
					},
					onChange: function(value) {
						if (value == '1') {
						...
						var myResult = checkPlugin('https://ilya-shambulin.github.io/mylampa.github.io/top.js');
                                                var pluginsArray = Lampa.Storage.get('plugins');
                                                    setTimeout(function() {
                                                       ...
                                                       if (myResult && pluginStatus !== 0) {
                                                          $('div[data-name="top"]').find('.settings-param__status').removeClass('active error').addClass('active');
                                                       } else if (pluginStatus === 0) {
                                                          $('div[data-name="top"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                                                       } else {
                                                          $('div[data-name="top"]').find('.settings-param__status').removeClass('active error').addClass('error');
                                                       }
                                                    }, 100);
						    item.on("hover:enter", function (event) {
                                                        nthChildIndex = focus_back(event); // Сохраняем элемент в переменной
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Nova Plus',
						description: 'Плагин меняет оформление для онлайн плагинов типа lampac'
					},
					onChange: function(value) {
						if (value == '1') {
						...
						var myResult = checkPlugin('https://ilya-shambulin.github.io/mylampa.github.io/nova_plus.js');
                                                var pluginsArray = Lampa.Storage.get('plugins');
                                                    setTimeout(function() {
                                                       ...
                                                       if (myResult && pluginStatus !== 0) {
                                                          $('div[data-name="nova_plus"]').find('.settings-param__status').removeClass('active error').addClass('active');
                                                       } else if (pluginStatus === 0) {
                                                          $('div[data-name="nova_plus"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                                                       } else {
                                                          $('div[data-name="nova_plus"]').find('.settings-param__status').removeClass('active error').addClass('error');
                                                       }
                                                    }, 100);
						    item.on("hover:enter", function (event) {
                                                        nthChildIndex = focus_back(event); // Сохраняем элемент в переменной
...
/* Управление */
		...
		Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://tsynik.github.io/lampa/e.js', 'Выход', '@tsynik', 'Exit_Menu', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nnmdd.github.io/lampa_hotkeys/hotkeys.js', 'Горячие Кнопки', '@nnmd', 'Hot_Buttons', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nemiroff.github.io/lampa/select_weapon.js', 'Тип Управления', '@nemiroff', 'Select_Weapon', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nb557.github.io/plugins/not_mobile.js', 'Выключение Тача', '@t_anton', 'Touch_off', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	       ...
	       /* Lampa.SettingsApi.addParam({
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('http://193.233.134.21/plugins/setprotect', 'Доступ к настройкам', '@AndreyURL54', 'setprotect', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('http://193.233.134.21/plugins/menusort', 'Сортировка Главного Меню', '@AndreyURL54', 'Sort_mainmenu', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://apxubatop.github.io/lmpPlugs/tvbuttontst.js', 'Бинд Кнопок', '@Juri_Z', 'bind_but', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	     ...
	     /*   Lampa.SettingsApi.addParam({
                ...
                Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'CUB Sync',
                                    		description: 'Синхронизация закладок и истории с CUB в локальное хранилище приложения'
					},
					onChange: function(value) {
						if (value == '1') {
	    ...
	    Lampa.SettingsApi.addParam({
					...
					field: {
                                    		...
                                    		description: 'Плагин создает в верхнем баре иконку для перезагрузки приложения'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Онлайн */
		...
		Lampa.SettingsApi.addParam({
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                  		name: 'Showy WTCH (free)',
                                  		description: 'Плагин для просмотра фильмов и сериалов в онлайн'
					},
                           		onChange: function(value) {
					if (value == '1') {
						itemON('http://wtch.ch/m', 'Showy WTCH (free)', '@showy', 'Showy WTCH', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
					}
					if (value == '2') {
						var pluginToRemoveUrl = "http://wtch.ch/m";
						deletePlugin(pluginToRemoveUrl, nthChildIndex);
							// console.log("nthChildIndex, переданный в deletePlugin:", nthChildIndex);
					}
                },
					onRender: function (item) {$('.settings-param__name', item).css('color','f3d900'); hideInstall();
						...
						var myResult = checkPlugin('http://wtch.ch/m');
                                                var pluginsArray = Lampa.Storage.get('plugins');
                                                    setTimeout(function() {
                                                       $('div[data-name="Showy WTCH"]').append('<div class="settings-param__status one"></div>');
                                                       var pluginStatus = null;
                                                       for (var i = 0; i < pluginsArray.length; i++) {
                                                          if (pluginsArray[i].url === 'http://wtch.ch/m') {
                                                             pluginStatus = pluginsArray[i].status;
                                                             break;
                                                          }
                                                       }
                                                       if (myResult && pluginStatus !== 0) {
                                                          $('div[data-name="Showy WTCH"]').find('.settings-param__status').removeClass('active error').addClass('active');
                                                       } else if (pluginStatus === 0) {
                                                          $('div[data-name="Showy WTCH"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                                                       } else {
                                                          $('div[data-name="Showy WTCH"]').find('.settings-param__status').removeClass('active error').addClass('error');
                                                       }
                                                    }, 100);	
						    item.on("hover:enter", function (event) {
                                                        nthChildIndex = focus_back(event); // Сохраняем элемент в переменной
              ...
              /*  Lampa.SettingsApi.addParam({
					...
					field: {
                           		...
                           		onChange: function(value) {
					if (value == '1') {
						itemON('http://smotret24.ru/online.js', 'Смотреть RU', '@showy', 'smotret_ru', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
					}
					if (value == '2') {
                ...
                Lampa.SettingsApi.addParam({
					...
					field: {
                                  		name: 'Смотреть 4K',
                                  		description: 'Плагин для просмотра фильмов и сериалов в онлайн, используется один стабильный источник в качестве 4К'
					},
                           		onChange: function(value) {
					if (value == '1') {
						itemON('http://smotretk.com/online.js', 'Смотреть 4K', '@showy', 'smotret_4k', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
					}
					if (value == '2') {
	   ...
	   /* Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Free Online',
						description: 'Плагин позволяет смотреть фильмы и сериалы в онлайн'
					},
					onChange: function(value) {
						if (value == '1') {
						...
						/*var myResult = checkPlugin('https://ilya-shambulin.github.io/mylampa.github.io/free_onl.js');
                                                var pluginsArray = Lampa.Storage.get('plugins');
                                                    setTimeout(function() {
                                                       ...
                                                       if (myResult && pluginStatus !== 0) {
                                                          $('div[data-name="free_onl"]').find('.settings-param__status').removeClass('active error').addClass('active');
                                                       } else if (pluginStatus === 0) {
                                                          $('div[data-name="free_onl"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                                                       } else {
                                                          $('div[data-name="free_onl"]').find('.settings-param__status').removeClass('active error').addClass('error');
                                                       }
                                                    }, 100);
						    item.on("hover:enter", function (event) {
                                                        nthChildIndex = focus_back(event); // Сохраняем элемент в переменной
...
/* Торрент */
		...
		Lampa.SettingsApi.addParam({
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('http://193.233.134.21/plugins/checker.js', 'Поиск Локального TorrServera', '@AndreyURL54', 'Check_server', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	         ...
	         Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://lampame.github.io/main/torrentmanager/torrentmanager.js', 'Закачка Торрентов', '@feliks', 'Torr_download', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://github.freebie.tom.ru/torrents.js', 'Вторая Кнопка Торренты', '@VitalikPVA', 'second_but_torrents', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://plugin.rootu.top/ts-preload.js', 'Визуализация Загрузки TS', '@rootu', 'visual_ts', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
...
/* Телеканалы */
		...
		Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://andreyurl54.github.io/diesel5/diesel.js', 'Дизель ТВ', '@AndreyURL54', 'Diesel', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
...
/* Радио */
		...
		Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('http://cub.red/plugin/radio', 'Радио Record', '@lampa', 'Record', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
                        			if (value == '1') {
							itemON('https://lampame.github.io/main/rradio.js', 'Радио Record Mod', '@GwynnBleiidd', 'Record_Mod', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
                     				}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
                        			if (value == '1') {
							itemON('https://tsynik.github.io/lampa/soma.js', 'Радио SomaFM', '@tsynik', 'Radio_Soma', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
                     				}
						if (value == '2') {
...
/* Клубника */
		...
		/*Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('https://bwa.to/s', 'Клубничка', '@rik', 'Sisi', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
	      ...
	      /*  Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
							itemON('http://bwa-cloud.cfhttp.top/sisi.js', 'Клубничка (Bwa Cloud)', '@rik', 'Nyam', nthChildIndex);
							// console.log("nthChildIndex, переданный в itemON:", nthChildIndex);
						}
						if (value == '2') {
